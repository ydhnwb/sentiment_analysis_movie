        ...
        try:
            if c.place is not None:
                ...
                if "rt" not in text_val:
                    if len(text_val) != 0:
                        secondcount += 1
                        csvWriter.writerow([secondcount,str(text_val),c.user.location,c.place.bounding_box.coordinates,c.place.full_name,c.place.country,c.place.country_code,c.place.place_type])
                        print("[INFO] Get a tweet : " + str(secondcount) + " = " + text_val)
